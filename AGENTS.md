...
- **Cross-module amendment — CentaurTeamId unification and game credential JWT expiry**: `CentaurTeamId` is now a single `string & { readonly __brand: 'CentaurTeamId' }` type across all modules, defined as the Convex `centaur_teams._id`. The separate `CentaurTeamDocId` type has been eliminated. All field names, JWT claim names, and `Agent` discriminated union fields that previously said `centaurTeamDocId` now say `centaurTeamId`. Module 04's `CentaurTeamRosterRow` no longer has a separate `centaurTeamDocId` field — `centaurTeamId` IS the Convex doc ID string. All `centaurTeamId: number` type annotations in Module 04 have been changed to string. The game credential JWT `exp` in Module 03 has been changed from `iat + 604800` (7 days) to `iat + 7200` (2 hours), with rationale updated to reflect that 2h is generous relative to game duration and that teardown + game status checks are the real temporal enforcement. Affected modules: 01, 02, 03, 04. Module 06 already uses `centaurTeamId: v.id("centaur_teams")` correctly and was not changed.
