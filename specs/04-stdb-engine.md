...
**04-REQ-070** *(negative)*: The runtime shall not treat any connection as having elevated privileges beyond those derived from its JWT `sub` claim role and CentaurTeam association. Captain status, timekeeper role, and other Convex-side operator role distinctions ([03], [08]) shall not be visible to or enforceable by the runtime.
...
**`game_config`** — Single-row table storing the game identifier, game seed, game-end callback URL, and all dynamic gameplay parameters.
...
interface InitializeGameParams {
  ...
  readonly dynamicGameplayParams: {
    readonly snakesPerTeam: number
    readonly maxHealth: number
...
4. **Write turn-0 historical records** [04-REQ-014b, 04-REQ-014c, 04-REQ-014d]:
   ...
   - Insert one `time_budget_states` row per CentaurTeam for turn 0 with `remainingBudgetMs = initialBudgetMs` [01-REQ-035].
...
6. **Initialize per-turn working state**: Insert one `centaur_team_turn_state` row per CentaurTeam with `declaredTurnOver = false` and `remainingClockMs = min(firstTurnTimeMs, initialBudgetMs + budgetIncrementMs)` per [01-REQ-037].
...
```
budgetMs  += config.budgetIncrementMs               // [01-REQ-036]
cap        = (T === 0) ? config.firstTurnTimeMs
                       : config.maxTurnTimeMs        // [01-REQ-037]
...
interface DynamicGameplayParams {
  readonly snakesPerTeam: number
  readonly maxHealth: number
  readonly maxTurns: number
  readonly hazardDamage: number
...
`DynamicGameplayParams` is the runtime-behaviour subset of `GameConfig` ([01] §3.3), excluding board generation parameters. All enum values use the numeric encoding from [01] §3.1. The `gameSeed` is the root seed generated by Convex during `generateBoardAndInitialState()` — it must be passed through to STDB for turn-seed derivation.

**DOWNSTREAM IMPACT**: [05] must construct `InitializeGameParams` during game-start orchestration and pass it to the STDB instance via `POST /v1/database/{name}/call/initialize_game`. The `gameSeed` and `gameEndCallbackUrl` fields are included in [02] §3.4's `SpacetimeDbInstanceLifecycle.provision.inputFromConvex`.
