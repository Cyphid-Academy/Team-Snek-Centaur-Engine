...
**08-REQ-020**: The bot parameters page shall be scoped to a specific hosted Centaur Team. It shall display and allow editing of the team's bot parameter record ([06-REQ-011]), comprising at minimum: the softmax global temperature and the **automatic submission time allocation** (the time allocation applied to a snake's auto-submission timer per [07-REQ-044]). *(Amended per 08-REVIEW-011 resolution: `defaultOperatorMode` removed — operator-mode is replaced by per-operator ready-state per §8.12; `turn0AutomaticTimeAllocationMs` removed — the chess-clock's existing turn-0 budget governs turn-0 timing without a separate auto-submission allocation.)*
...
**08-REQ-027f**: The Room Lobby view shall expose, exclusively to members of an enrolled Centaur Team, affordances to mark that team ready and to unmark that team ready, consistent with the ready-check semantics of [05-REQ-031] (see 08-REVIEW-013 for the interaction with 05-REVIEW-007). The affordance shall be available to any team member whom [05] permits to mark the team ready; the requirement is technology-agnostic with respect to which exact role holds this capability, deferring to Module 05's eventual resolution.
...
**08-REQ-027i**: The Room Lobby view shall provide a **Board Preview** affordance that renders a miniature visualisation of the board geometry — including approximate placement of fertile tiles, hazards, and snake starting territories — derived from the room's current game-configuration parameter values. The preview shall regenerate as the administrative actor edits parameters; the regeneration cadence is a design-level concern. See 08-REVIEW-014.

**08-REQ-027j**: The Board Preview shall expose an affordance by which the administrative actor may **lock in** a specific generated preview to be used as the exact starting layout for the next game created in the room. When a preview is locked in, Convex shall be informed of that intent such that the subsequent game-start orchestration of [05-REQ-032] seeds the SpacetimeDB instance with the locked-in layout rather than a freshly generated one. See 08-REVIEW-015 for the unresolved question of where in the Module 05 schema the locked-in preview lives.

**08-REQ-027k** *(negative)*: The Board Preview shall not stage, commit, or otherwise affect the currently-playing game in the room. Board Preview affects only the *next* game-start invocation, consistent with the immutable-parameter-snapshot rule of [05-REQ-024].
...
**08-REQ-032**: The header of the live operator interface shall display at minimum: the current turn number, the team clock countdown, the team's remaining time budget, the measured network latency to the team's SpacetimeDB instance, a Convex-hosted presence display of other operators currently connected to the same game from the same team along with each connected operator's current ready-state per [06-REQ-040b] (per §8.12), and — conditionally, per §8.14 — the Captain control affordances. *(Amended per 08-REVIEW-011 resolution: team-level operator-mode indicator removed; per-operator ready-state shown in the operator presence display instead.)*
...
**08-REQ-034** *(removed per 08-REVIEW-011 resolution)*: The team-level operator-mode indicator no longer exists. Per-operator ready-state — which has replaced operator-mode as the per-turn coordination primitive — is rendered in the operator presence display per [08-REQ-032] and [06-REQ-040b]. The number is reserved to preserve stable cross-references; no requirement is currently associated with it.
...
*(Section retitled and rewritten per 08-REVIEW-011 resolution: the prior team-level operator-mode (Centaur / Automatic) model is replaced by per-operator ready-state. There is no longer a single team-level mode; every operator currently connected to the team's game session independently signals readiness, and the team's turn is declared over once a unanimity condition holds.)*
...
**08-REQ-062**: The Snek Centaur Server shall declare the team's turn over ([01-REQ-039]) when **all currently-connected operators on the team have set their ready-state to `ready` for the current turn** AND the bot-framework's compute queue is cleared for all owned snakes. If the team has zero currently-connected operators, declaration is deferred until at least one operator is connected (the per-snake auto-submission timer of [07-REQ-044] continues to apply independently per snake; the team's overall turn-over declaration awaits operator ready-state once an operator connects). Ready-state is reset to `not-ready` for every operator at the start of each new turn (i.e., on the publish of the next authoritative pre-turn board state per [04]). *(Amended per 08-REVIEW-011 resolution.)*
...
**08-REQ-064**: The initial ready-state of every operator at the start of every turn shall be `not-ready`. There is no team-level default that carries over from `global_centaur_params` — `defaultOperatorMode` no longer exists. *(Amended per 08-REVIEW-011 resolution.)*
...
**08-REQ-068**: Per-operator ready-state toggles shall produce an `operator_ready_toggled` entry in the action log ([06-REQ-036]) per [06-REQ-040b]. Turn submissions issued by the Captain shall produce the team-side turn-submission event category of [06-REQ-036]. *(Amended per 08-REVIEW-011 resolution: the legacy `mode_toggled` event is removed in favour of `operator_ready_toggled`.)*
...
**08-REQ-084**: The Live Spectating view shall display a **scoreboard** per participating team, showing at minimum each team's aggregate alive-snake length (as a proxy for team score pending resolution of 05-REVIEW-006) and each alive snake's current length and health. The scoreboard shall update live in response to subscription deliveries.
...
**08-REQ-097**: The Team Profile view shall resolve historical opponent attributions using the participating-team snapshots of [05-REQ-029] so that head-to-head records remain stable even if an opposing team has since been deleted per [05-REQ-015a].
...
**08-REQ-094e**: The Leaderboard view shall resolve historical attributions using the participating-team snapshots of [05-REQ-029], so that a team's ranking continues to reflect games it played under its historical identity even if the team has since been deleted per [05-REQ-015a]. See 08-REVIEW-017 on whether deleted teams should continue to appear in the leaderboard.

**08-REQ-094f** *(negative)*: The Leaderboard view shall not be accessible in a way that exposes team or player data to unauthenticated visitors. Leaderboard access is subject to the authentication requirement of [08-REQ-006]. See 08-REVIEW-016.
...
- The team's turn is declared over (per [01-REQ-039]) when **all currently-connected operators are simultaneously `ready` AND the bot framework's compute queue is empty for all owned snakes** (per [08-REQ-062]). The Captain may override this via the explicit turn-submit affordance of [08-REQ-065]. Otherwise, the team's existing per-turn clock and time budget ([01-REQ-037], [01-REQ-038]) continue to govern the upper bound on turn duration.
- The per-snake auto-submission timer of [07-REQ-044] (renamed semantics: it is the time from the snake having been auto-staged before the framework re-flushes any dirty bot computation; manual-mode snakes are still excluded per [07-REQ-046]) continues to operate independently per snake. There is no separate team-level submission timer.
- The `defaultOperatorMode` and `turn0AutomaticTimeAllocationMs` fields are removed from `global_centaur_params` and `game_centaur_state` (per [06-REQ-011] and [06-REQ-040a]). The remaining `automaticTimeAllocationMs` field is retained with its renamed per-snake semantics. The action-log event `mode_toggled` is removed and replaced by `operator_ready_toggled` (per [06-REQ-040b]).
...
- **Module 07**: 07-REQ-044 / 07-REQ-045 / 07-REQ-046 amended — per-snake `automaticTimeAllocationMs` semantics retained but reframed as a per-snake auto-submission timer that is no longer team-level; 07-REQ-045a amended (no separate turn-0 allocation; turn-0 timing follows the chess-clock's existing turn-0 budget); §2.13 final-flush rules updated.
...
### 08-REVIEW-013: Who marks a team ready
...
### 08-REVIEW-014: Board preview generation locality
...
### 08-REVIEW-015: Where a locked-in board preview is persisted
...
### 08-REVIEW-016: Public vs authenticated-only visibility of profiles and leaderboard
...
### 08-REVIEW-017: Deleted teams in leaderboards
...
### 08-REVIEW-018: Live spectating when invisibility is active
...
### 08-REVIEW-019: Timeline scrubber data delivery for long games
...
### 08-REVIEW-020: Game-in-progress discoverability on the home view
