...
- `not-started → playing`: Config is frozen (02-REQ-050), fresh SpacetimeDB instance provisioned (02-REQ-019, 02-REQ-020), game invitations sent to nominated servers and accepted.
...
**DOWNSTREAM IMPACT**: [04] must implement the `initialize_game` reducer to accept these inputs. [05] must implement the provisioning orchestration that supplies them, including the game-start invitation flow to nominated servers. [03] must define the game credential generation and the admission ticket format that SpacetimeDB validates.
